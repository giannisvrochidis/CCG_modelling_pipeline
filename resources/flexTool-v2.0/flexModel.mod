...
param use_capacity_margin 'Flag: Additional capacity margin over the peak demand in the investment model time series' default 1;
param use_inertia_limit 'Flag: Inertia limit - there has to be enough inertia in the nodeGroup at all times' default 1;
param use_ramps 'A flag to indicate whether ramp constraints are in use or not' default 1;
param use_online 'A flag to indicate whether online variables (startups and min.loads) are in use' default 1;
param use_non_synchronous 'A flag to indicate whether to use non-synchronous constraint' default 1;
param mode_invest 'A flag to indicate that investments are to be optimized' default 0;
param mode_dispatch 'A flag to indicate that dispatch is to be optimized' default 1;  # 0 = no dispatch, 1 = only dispatch, 2 = dispatch after investment solve
param decs_round_invest 'Number of decimals in the investment results' default 3;
param loss_of_load_penalty 'Penalty for violating the energy balance constraint' default 10000;
param curtailment_penalty 'Cost for curtailing variable generation' default 0;
param loss_of_reserves_penalty 'Penalty for violating the reserve constraints' default 20000;
param lack_of_inertia_penalty 'Penalty for violating the reserve constraints' default 30000;
param lack_of_capacity_penalty 'Penalty for violating the capacity margin constraint' default 5000;

#########################
# Time sets and parameters defining the model temporal structure
param time_in_years 'The length of the time series in years (not time_in_use, but the whole time series)';
param time_period_duration 'The duration of a single time period in minutes' default 60;
...
param p_time_jump {t in time} := (if not mode_invest then p_time_jump_dispatch[t] else p_time_jump_invest[t]);
...
param reserve_duration 'For how long reserve needs to able to serve - affects how much storages need to have stored energy';
...
param co2_cost;
param print_duration default 1;
param print_durationRamp default 1;
param print_unit_results default 1;
...
#########################
# Combanitorial sets
set nodeNode 'Existing transmission links and links with investment possibility' := {n in node, n2 in node : sum {g in grid} p_nodeNode[g, n, n2, 'cap.rightward_MW'] || sum {g in grid} p_nodeNode[g, n2, n, 'cap.leftward_MW'] || sum {g in grid} p_nodeNode[g, n, n2, 'max_invest_MW'] || sum {g in grid} p_nodeNode[g, n2, n, 'max_invest_MW']};
set nodeNode_invest 'Transmission links with investment possibility' := {n in node, n2 in node : mode_invest && sum {g in grid} p_nodeNode[g, n, n2, 'max_invest_MW']};
set gn := {g in grid, n in node : p_node[g, n, 'demand_MWh'] || sum{u in unit} p_unit[g, n, u, 'capacity_MW'] || sum{u in unit} p_unit[g, n, u, 'storage_MWh'] || 
            sum {n2 in node} p_nodeNode[g, n, n2, 'cap.rightward_MW'] || sum {n2 in node} p_nodeNode[g, n2, n, 'cap.leftward_MW'] || sum {n2 in node} p_nodeNode[g, n, n2, 'max_invest_MW'] || sum {n2 in node} p_nodeNode[g, n2, n, 'max_invest_MW']};
...
set gnn := {g in grid, n in node, n2 in node : (p_nodeNode[g, n, n2, 'cap.rightward_MW'] || p_nodeNode[g, n, n2, 'cap.leftward_MW'] || p_nodeNode[g, n, n2, 'max_invest_MW'] || p_nodeNode[g, n, n2, 'invested_capacity_MW'])};
...
param casefolder symbolic;         
param resfolder symbolic;
param tsfolder symbolic;
...
table infile7 IN 'CSV' tsfolder & 'ts_time.csv':
time <- [time], p_time_dispatch~in_use, p_time_jump_dispatch~time_jump, p_time_invest~in_use_invest, p_time_jump_invest~time_jump_invest;

table infile0 IN 'CSV' tsfolder & 'ts_energy.csv':
I <- [grid,node,time], ts_energy~value;

table infile1 IN 'CSV' tsfolder & 'ts_import.csv':
J <- [grid,node,time], ts_import~value;

table infile2 IN 'CSV' tsfolder & 'ts_cf.csv':
K <- [cf_profile,time], ts_cf~value;

table infile3 IN 'CSV' tsfolder & 'ts_inflow.csv':
L <- [inflow,time], ts_inflow~value;

table infile4 IN 'CSV' tsfolder & 'ts_reserve_node.csv':
M <- [node,time], ts_reserve_n~value;

table infile5 IN 'CSV' tsfolder & 'ts_reserve_nodeGroup.csv':
N <- [nodeGroup,time], ts_reserve_ng~value;

table infile6 IN 'CSV' tsfolder & 'ts_unit.csv':
O <- [grid,node,unit,unit_ts_param,time], ts_unit~value;
...
s.t. transferLimit_rightward {(g, n, n2, t) in gnnt} :
  ...
  + (if (n, n2) in nodeNode_invest then v_investTransfer[g, n, n2])
;
...
param netLoadRamp{(g,n,t) in gnt : (g,n) in gn_demand && p_time_jump[t] = 1} := 
  ( (demand[g,n,t] - demand[g,n,t-p_time_jump[t]])
    - (if (g,n) in gn_import then (import[g,n,t] - import[g,n,t-p_time_jump[t]]))
    - sum {(g,n,u) in gnu_cf union gnu_inflow_noStorage} (v_gen[g,n,u,t].val - v_gen[g,n,u,t-p_time_jump[t]].val)
  );
...
param netLoadRamp_4h{(g,n,t) in gnt : (g,n) in gn_demand && p_time_jump[t] = 1} :=
  ( (demand[g,n,t] - demand[g,n,t-p_time_jump[t-p_time_jump[t-p_time_jump[t-p_time_jump[t]]]]])
    - (if (g,n) in gn_import then (import[g,n,t] - import[g,n,t-p_time_jump[t-p_time_jump[t-p_time_jump[t-p_time_jump[t]]]]]))
    - sum {(g,n,u) in gnu_cf union gnu_inflow_noStorage} (v_gen[g,n,u,t].val - v_gen[g,n,u,t-p_time_jump[t-p_time_jump[t-p_time_jump[t-p_time_jump[t]]]]].val)
  );
...
param fn_colors{g in grid} symbolic := resfolder & casefolder & "color_codes_" & g & ".dat";
for {g in grid}	printf '#Color codes for %s\n', g > fn_colors[g];
param fn_colors_no_grid symbolic := resfolder & casefolder & "color_codes_no_grid.dat";
printf '#Color codes for items not dependent on the grid\n' > fn_colors_no_grid;

#########################
# Outputs to result csv files

table transfers_over_time {(g, n, n2, t) in gnnt} OUT 'CSV' resfolder & casefolder & 'transfers_t_' & (if mode_invest then "I.csv" else "D.csv"): 
  g~grid, n~node, n2~node, t~time, 
  - v_transfer[g,n,n2,t].val * p_nodeNodeEff[g,n,n2] ~rightward_MWh
;

table unit_investment_results {(g, n, u) in gnu_all} OUT 'CSV' resfolder & casefolder & 'units_invest_' & (if mode_invest then "I.csv" else "D.csv"): 
...
table transfers {(g, n, n2) in gnn} OUT 'CSV' resfolder & casefolder & 'transfers_invest_' & (if mode_invest then "I.csv" else "D.csv"): 
...
param fn_events symbolic := resfolder & casefolder & "events_" & (if mode_invest then "I.csv" else "D.csv");
...
param fn_summary symbolic := resfolder & casefolder & "summary_" & (if mode_invest then "I.csv" else "D.csv");
...
param fn_unit{g in grid} symbolic := resfolder & casefolder & "units_" & g & (if mode_invest then "_I.csv" else "_D.csv");
for {g in grid}
  {
...
param fn_node symbolic := resfolder & casefolder & "node_" & (if mode_invest then "I.csv" else "D.csv");
...
param fn_transfer{g in grid} symbolic := resfolder & casefolder & "transfers_" & g & (if mode_invest then "_I.csv" else "_D.csv");
...
param fn_genUnitGroup{g in grid} symbolic := resfolder & casefolder & "genUnitGroup_" & g & (if mode_invest then "_I.csv" else "_D.csv");
for {g in grid}
  {
...
param fn_genUnit{g in grid} symbolic := resfolder & casefolder & "genUnit_" & g & (if mode_invest then "_I.csv" else "_D.csv");
for {g in grid}
  {
...
param fn_state{g in grid} symbolic := resfolder & casefolder & "storageContent_" & g & (if mode_invest then "_I.csv" else "_D.csv");
for {g in grid}
  {
...
param fn_onlineUnit{g in grid} symbolic := resfolder & casefolder & "onlineUnit_" & g & (if mode_invest then "_I.csv" else "_D.csv");
...
param fn_inertiaUnit{g in grid} symbolic := resfolder & casefolder & "inertiaUnit_" & g & (if mode_invest then "_I.csv" else "_D.csv");
...
param fn_reserveUnit{g in grid} symbolic := resfolder & casefolder & "reserveUnit_" & g & (if mode_invest then "_I.csv" else "_D.csv");
...
param fn_gnt{(g,n) in gn} symbolic := resfolder & casefolder & "node_t_" & g & "_" & n & (if mode_invest then "_I.csv" else "_D.csv");
...
param fn_gt{g in grid} symbolic := resfolder & casefolder & "grid_t_" & g & (if mode_invest then "_I.csv" else "_D.csv");
for {g in grid}
  {
...
param fn_costs symbolic := resfolder & casefolder & "costs_" & (if mode_invest then "I.csv" else "D.csv");
...
param fn_costs_unitGroup symbolic := resfolder & casefolder & "costs_unitGroup_" & (if mode_invest then "I.csv" else "D.csv");
...
param fn_costs_time symbolic := resfolder & casefolder & "costs_t_" & (if mode_invest then "I.csv" else "D.csv");
...
param fn_duration{g in grid} symbolic := resfolder & casefolder & "duration_" & g & (if mode_invest then "_I.csv" else "_D.csv");
...
param fn_durationRamp{g in grid} symbolic := resfolder & casefolder & "durationRamp_" & g & (if mode_invest then "_I.csv" else "_D.csv");
...
param fn_rampRoom_1h{(g, n) in gn} symbolic := resfolder & casefolder & "rampRoom_1h_" & g & "_" & n & (if mode_invest then "_I.csv" else "_D.csv");
...
param fn_rampRoom_4h{(g, n) in gn} symbolic := resfolder & casefolder & "rampRoom_4h_" & g & "_" & n & (if mode_invest then "_I.csv" else "_D.csv");
for {(g,n) in gn : p_node[g,n,'print_results'] && (g,n) in gn_demand && use_ramps}
  {
    ...
    for {t in time_in_use : p_time_jump[t] = 1 && p_time_jump[t-p_time_jump[t]] = 1 && p_time_jump[t-p_time_jump[t-p_time_jump[t]]] = 1 && p_time_jump[t-p_time_jump[t-p_time_jump[t-p_time_jump[t]]]] = 1}
      {
...
param fn_rampRoom_1h_grid{g in grid} symbolic := resfolder & casefolder & "rampRoom_1h_" & g & (if mode_invest then "_I.csv" else "_D.csv");
...
param fn_rampRoom_4h_grid{g in grid} symbolic := resfolder & casefolder & "rampRoom_4h_" & g & (if mode_invest then "_I.csv" else "_D.csv");
for {g in grid : sum{(g,n) in gn_demand} 1 && use_ramps}
  {
...
param fn_invested symbolic := resfolder & casefolder & "investments_" & (if mode_invest then "I.dat" else "D.dat");
printf "data;\n\n" > fn_invested;
